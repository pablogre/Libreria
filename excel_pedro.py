...
for r in range(1, sheet.nrows):
    ...
    codigo = str(sheet.cell(r, 0).value)
    barras = ''
    articulo = sheet.cell(r, 1).value
    id_rubro = 1
    
    precio1 = float(str(sheet.cell(r, 2).value))

    if str(sheet.cell(r, 3).value) != "":
        precio2 = float(str(sheet.cell(r, 3).value))
    ...
    query = 'SELECT * FROM articulos where id_empresa = %s and  codigo like %s'
    ...
    if data:
        cur = connection.cursor()
        query = "update articulos set precio1 = %s, precio2 = %s where codigo = %s and id_empresa = %s"
        params = [precio1, precio2, codigo, id_empresa]
        cur.execute(query, params)
        connection.commit()
    else:
        # Obtener el objeto del cursor, utilizado para recorrer los datos de la base de datos fila por fila
